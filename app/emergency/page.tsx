...
/**
 * EmergencyPage — manual system broadcast triggers and evacuation routing controls.
 */
export default function EmergencyPage() {
  const [panicActive, setPanicActive] = useState(false);
  const [evacRoute, setEvacRoute] = useState('Route Alpha (North Gate)');

  const handlePanicToggle = useCallback(() => {
  ...
  const routes = ['Route Alpha (North Gate)', 'Route Beta (East Gate)', 'Route Gamma (South Gate)'];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 max-w-7xl">
        ...
        <div className="lg:col-span-2 bento-card bg-white p-6 flex flex-col justify-between items-center text-center">
          ...
          <button
            ...
            aria-label={panicActive ? 'Shutdown emergency protocol' : 'Trigger emergency broadcast'}
          ...
          <span className="text-xs font-bold text-black uppercase" role="status" aria-live="polite">
        ...
        <div className="lg:col-span-2 bento-card bg-black text-white p-6 flex flex-col justify-between">
          <div>
            ...
            <div className="space-y-4 mt-6" role="radiogroup" aria-label="Evacuation routing selector">
              {routes.map((route) => {
                const isSelected = evacRoute === route;
                return (
                  <div
                    key={route}
                    ...
                    className={`p-4 border-4 border-white cursor-pointer uppercase font-bold text-sm outline-none focus-visible:border-[#E2FF32] focus-visible:ring-2 focus-visible:ring-[#E2FF32] transition-colors ${
                      isSelected ? 'bg-[#E2FF32] text-black' : 'bg-gray-900 text-white'
                    }`}
                  >
                    {route}
                  </div>
          ...
          <div className="mt-8 p-4 bg-gray-900 border-2 border-white text-xs font-mono text-green-400" role="status" aria-live="polite">
